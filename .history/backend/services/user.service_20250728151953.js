...
async function createUser(user) {
  console.log(user);
    ...
    try {
        ...
        const query = "INSERT INTO users (email, phone_number, password, role) VALUES (?, ?, ?, ?)";
        const rows = await conn.query(query, [user.email, user.phone_number, hashedPassword, user.role]);
        
        if (rows.affectedRows !== 1) {
            return false;
        }
        const user_id = rows.insertId;
        createdUser = { user_id, email: user.email, phone_number: user.phone_number, role: user.role };
    } catch (error) {
        console.log(error);
    }
    return createdUser;
